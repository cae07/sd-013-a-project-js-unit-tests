...
let createMenu = (menu) => {
  const order = (item) => {
    ...
    const close = () => {
      let price = Object.values(restaurant.fetchMenu());
      let bill = restaurant.consumption;
      let total = null;
      bill.forEach((item) => { 
        total += price.find((obj) => obj[item])[item];
      });
      return total;
  };
  restaurant.pay = () => close();
  return restaurant;
};
