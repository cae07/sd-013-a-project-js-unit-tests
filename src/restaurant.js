...
function payment(input) {
  const menus = Object.values(input);
  const price = {};
  const eated = menu.consumption;
  let total = 0;
  for (let i = 0; i < menus.length; i += 1) {
    Object.assign(price, menus[i]);
  }
  for (let i = 0; i < eated.length; i += 1) {
    total += price[eated[i]];
  }
...
const createMenu = (input) => {
  menu = {
    fetchMenu: () => input,
    consumption: [],
    order: (order) => clientOrder(order),
    pay: () => payment(input),
  };
  return menu;
}; 
