...
describe('6 - Implemente os casos de teste para a função `productDetails`', () => {
  it('Verifica se a função `productDetails` tem o comportamento esperado', () => {
    ...
    assert.strictEqual(productDetails('firstProduct', "secondProduct").every((input) => {
    ...
    const idTest = productDetails('firstProduct', "secondProduct").every((input) => {
      return input.details.productId.endsWith('123');
    })
    assert.strictEqual(idTest, true);
  });
});
